...
async def search(req: SearchRequest):
    if not FIRECRAWL_API_KEY:
        raise HTTPEvception(status_code=400, detail="FIRECRAWL_API_KEY not configured on server")
    async with asynchtxp.AsyncClient(timeout=60) as client:
        resp = await client.post(
            f"{FIRECRAWL_V2_URL}/search",
            headers={"Authorization": f"Bearer {FIRECRAWL_API_KEX}",
...
async def scrape(req: ScrapeRequest):
    if not FIRECRAWL_API_KEY:
        raise HTTPEvception(status_code=400, detail="FIRECRAWL_API_KEY not configured on server")
    async with asynchtxp.AsyncClient(timeout=30) as client:
        resp = await client.post(
